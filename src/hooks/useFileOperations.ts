...
## ✨ 0.2.2 の主な変更
- アプリ終了時に設定が保存されないケースの修正
- エディット有効化を（"Edit On"）のトグルボタンに変更
- OSSポリシーの明確化
...
### 表示モード
- **プレビューのみ**: プレビュー単独表示（デフォルト）
- **分割ビュー**: エディター + プレビュー（Edit On時）

## 🎨 カスタマイズ
- テーマ: GitHub Light / GitHub Dark / Auto
  ...
  const handleSave = useCallback(async () => {
    if (isTauri) {
      ...
      const blob = new Blob([markdownContent], {
        type: APP_CONFIG.DOWNLOAD_MIME_TYPE,
      })
  ...
  const handleTauriFileDrop = useCallback(
    async (filePath: string) => {
      try {
        ...
        if (!readTextFile) {
          logger.warn(
            'Tauri file API not available, but file drop functionality is working through alternative path'
          )
          return
        }
